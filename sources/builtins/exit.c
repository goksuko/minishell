...
void	handle_numeric_arg(t_tree **ast, t_env **env_var)
{
	...
	if (arg_is_digit((*ast)->argument[1]) == false)
	{
		free_tree_env(ast, env_var);
	...
	{
		exit_code = exit_atoi((*ast)->argument[1], ast, env_var);
		free_tree_env(ast, env_var);
		exit(exit_code);
	}
}
...
{
	if ((*ast)->argument[1] == NULL)
		handle_no_args(ast, env_var);
	else if ((*ast)->argument[2] != NULL)
		handle_too_many_args(ast, env_var);
	else if ((*ast)->argument[1] != NULL)
