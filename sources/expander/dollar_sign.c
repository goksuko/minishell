...
char *get_dollar_replacement(t_data **data, char *str, int i, int *end_dollar)
{
	if (str[i + 1] == '?')
...
char *handle_dollar_sign(t_data **data, char *str)
{
	...
	while (str[i] != '\0') {
		if (str[i] == '$') {
			char *middle_to_add = get_dollar_replacement(data, str, i, &end_dollar);
			if (middle_to_add == NULL) {
				free(temp);
				return NULL;
			}
			char *new_temp = ft_strjoin(temp, middle_to_add);
			free(temp);
			free(middle_to_add);
			if (new_temp == NULL)
				return NULL;
			...
			char *new_temp = ft_strjoin_c(temp, str[i]);
			free(temp);
			if (new_temp == NULL)
				return NULL;
			temp = new_temp;
