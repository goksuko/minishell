...
char	*expand_identifier(t_data **data, char *argument)
{
	...
	{
		free_system_perror(*data, ERROR_ALLOCATION, "expanded_str in expand_identifier");
		return (NULL);
	}
	if (dollar_sign_check(expanded_str) == true)
	{
		...
		if (errno == ENOMEM || expanded_str == NULL)
		{
			free_system_perror(*data, ERROR_ALLOCATION, "handle dollar sign in expand_identifier");
			return (NULL);
		}
	}
	return (expanded_str);
